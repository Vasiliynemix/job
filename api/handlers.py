...
async def delete_user(
    ...
    if user_for_deletion is None:
        raise HTTPException(
            status_code=404, detail=f"Users with id {user_id} not found."
        )
...
@router_user.patch("/admin_privileges", response_model=UpdatedUserResponse)
async def grant_admin_privilege(
    ...
    if current_user.user_id == user_id:
        raise HTTPException(
            status_code=400, detail="Cannot manage privileges if itself."
        )
    user_for_promotion = await _get_user_by_id(user_id, db)
    if user_for_promotion.is_admin or user_for_promotion.is_superadmin:
        raise HTTPException(
            status_code=409,
            detail=f"User with id {user_id} already promoted admin / superadmin.",
        )
...
@router_user.delete("/admin_privileges", response_model=UpdatedUserResponse)
